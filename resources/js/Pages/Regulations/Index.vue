...
<template>
  <AuthenticatedLayout>
    <div class="relative flex flex-col md:flex-row overflow-x-hidden min-h-screen">
      ...
      <div class="relative flex-1 min-w-0 z-10 bg-white/40 dark:bg-gray-900/40 min-h-full">
        <div class="p-6 sm:p-8 max-w-5xl mx-auto text-gray-900 dark:text-gray-100">
          
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white"> Liste des règlements</h1>
              <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">Consultez et gérez les règlements de l'organisation</p>
            </div>
...
hover:shadow-red-500/30 transform hover:scale-105 transition-all"
          ...
          <div v-if="filteredTitles.length === 0" class="text-center py-12 bg-white/30 dark:bg-gray-800/60 backdrop-blur-md rounded-lg border border-gray-200 dark:border-gray-700 transition-colors">
            <i class="fas fa-search text-4xl text-gray-400 dark:text-gray-500 mb-4"></i>
            <p class="text-gray-700 dark:text-gray-200 font-semibold text-lg mb-2">Aucun règlement trouvé</p>
            <p class="text-gray-600 dark:text-gray-400 text-sm">"{{ search }}" ne correspond à aucun règlement</p>
          </div>

          <div v-else
              class="relative bg-white/30 dark:bg-gray-800/60 backdrop-blur-md shadow-xl border border-gray-200 dark:border-gray-700 rounded-lg p-3 cursor-grab active:cursor-grabbing overflow-hidden transition-colors"
              @mouseenter="stop"
              @mouseleave="start"
          ...
          >
              ...
              <div v-for="t in filteredTitles" :key="t.id" class="w-full flex-shrink-0 px-2 sm:px-4 box-border">
                ...
                <ul class="space-y-4">
                  <li v-for="c in t.contents" :key="c.id" 
                      class="border border-gray-300 dark:border-gray-600 p-4 sm:p-5 rounded-xl bg-white/70 dark:bg-gray-700/70 backdrop-blur-sm animate-slideUp hover:shadow-lg transition-shadow duration-300">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                      <div class="flex-1">
                        ...
                        <ul class="space-y-2">
                          <li v-for="line in contentLines(c.content)" :key="line" 
                              class="text-gray-900 dark:text-gray-100 flex items-start gap-2">
                      ...
                      <div class="flex gap-2 mt-4 sm:mt-0">
                        ...
                        <button v-if="$page.props.auth.user.role === 'admin'"
                               class="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-red-600 to-pink-600 rounded-lg text-white font-bold text-sm shadow-lg hover:shadow-red-500/30 transform hover:scale-105 transition-all"
            ...
            <button @click="prev" class="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-white/80 dark:bg-gray-700/80 text-citron-700 dark:text-citron-400 p-2 rounded-full shadow-lg hover:bg-white/90 dark:hover:bg-gray-700 transition">
              <i class="fas fa-chevron-left text-lg"></i>
            </button>
            <button @click="next" class="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-white/80 dark:bg-gray-700/80 text-citron-700 dark:text-citron-400 p-2 rounded-full shadow-lg hover:bg-white/90 dark:hover:bg-gray-700 transition">
            ...
            <div class="flex gap-2 justify-center mt-6 sm:mt-8">
              <button v-for="(t, i) in filteredTitles" :key="t.id"
                      ...
                      :class="i === current ? 'bg-citron-600 dark:bg-citron-400 scale-110' : 'bg-gray-400 dark:bg-gray-500 hover:bg-gray-500 dark:hover:bg-gray-400'"
                      @click="goTo(i)"
    ...
    <transition name="fade">
      <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 max-w-lg w-full rounded-lg p-6 shadow-2xl z-10 transition-colors">
          ...
          <div class="flex justify-end gap-3">
            <button class="px-4 py-2 rounded border border-gray-300 dark:border-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" @click="closeDeleteModal">
              Annuler
            </button>
            <button class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded transition-colors" @click="deleteRegulationConfirm">
              Supprimer
            </button>
          </div>
        </div>
    ...
    <transition name="fade">
      <div v-if="showContentDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 max-w-md w-full rounded-lg p-6 shadow-2xl z-10 transition-colors">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Confirmer la suppression</h3>
          <p class="text-sm text-gray-700 dark:text-gray-300 mb-4">
            Voulez-vous vraiment supprimer <strong class="text-gray-900 dark:text-white">{{ selectedContentLabel
              }}</strong> ?
          </p>

          <div class="flex justify-end gap-3">
            <button class="px-4 py-2 rounded border border-gray-300 dark:border-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              @click="closeContentDeleteModal">
              Annuler
            </button>
            <button class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded transition-colors" @click="deleteContentConfirm">
              Supprimer
            </button>
          </div>
        </div>
