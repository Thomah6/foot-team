        ...
        <div class="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-black text-white font-sans relative overflow-hidden">
            ...
            <div class="relative bg-gradient-to-b from-slate-800 to-slate-900 py-12 px-4 border-b border-slate-700">
                ...
                <div class="flex items-center justify-between max-w-6xl mx-auto mb-12">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-16 h-16 bg-gradient-to-br from-slate-700 to-slate-800 rounded-xl flex items-center justify-center border border-slate-600 hover:scale-110 transition-transform duration-300">
                            <i class="fas fa-medal text-white text-2xl animate-bounce-slow"></i>
                        </div>
                ...
                <div class="text-center mb-8">
                    <h1 class="text-6xl font-thin text-white tracking-wider mb-3 animate-fade-in">GÉNÉRAL</h1>
                    <div class="text-slate-400 text-lg tracking-widest font-light">CLASSEMENT GÉNÉRAL</div>
                </div>
            </div>
            ...
            <div class="max-w-5xl mx-auto px-4 py-12 relative z-10">
                ...
                <div v-if="props.classement.length === 0" class="text-center py-32">
                    <div class="animate-float-slow">
                        <i class="fas fa-medal text-slate-700 text-7xl mb-8"></i>
                    </div>
                ...
                <div v-else-if="props.classement.length >= 3" class="mb-16">
                    <div class="flex justify-center items-end gap-8 mb-8">
                        ...
                        <div class="flex flex-col items-center animate-slide-up-delay-2">
                            <div class="bg-gradient-to-br from-slate-600 to-slate-700 w-28 h-28 rounded-2xl flex items-center justify-center mb-4 border border-slate-600/30 shadow-2xl">
                                <span class="text-4xl font-thin text-white">2</span>
                            </div>
                            <img :src="props.classement[1].avatar || 'https://via.placeholder.com/100x100.png/C0C0C0?text=2'" 
                                 class="w-16 h-16 rounded-full border-3 border-slate-600 mb-2 transform transition-all duration-300 hover:scale-110 hover:rotate-6" />
                            <p class="text-white font-light text-lg">{{ props.classement[1].name }}</p>
                            <p class="text-slate-400 text-sm">{{ props.classement[1].pseudo }}</p>
                            <div class="mt-2">
                                <p class="text-slate-300 text-sm">B: {{ props.classement[1].total_goals }} | P: {{ props.classement[1].total_assists }}</p>
                                <p class="text-2xl font-thin text-white mt-1">{{ props.classement[1].points }} pts</p>
                            </div>
                        </div>

                        <!-- 1ère place -->
                        <div class="flex flex-col items-center animate-slide-up-delay">
                            <div class="bg-gradient-to-br from-yellow-600 to-yellow-700 w-32 h-32 rounded-2xl flex items-center justify-center mb-4 border border-yellow-600/30 shadow-2xl animate-glow">
                                <span class="text-5xl font-thin text-black">1</span>
                            </div>
                            <img :src="props.classement[0].avatar || 'https://via.placeholder.com/100x100.png/FFD700?text=1'" 
                                 class="w-20 h-20 rounded-full border-4 border-yellow-600/50 mb-2 transform transition-all duration-300 hover:scale-110 hover:rotate-6" />
                            <p class="text-white font-light text-xl">{{ props.classement[0].name }}</p>
                            <p class="text-slate-400 text-sm">{{ props.classement[0].pseudo }}</p>
                            <div class="mt-2">
                                <p class="text-yellow-500 text-sm">B: {{ props.classement[0].total_goals }} | P: {{ props.classement[0].total_assists }}</p>
                                <p class="text-3xl font-thin text-yellow-500 mt-1">{{ props.classement[0].points }} pts</p>
                            </div>
                        </div>

                        <!-- 3ème place -->
                        <div class="flex flex-col items-center animate-slide-up-delay-3">
                            <div class="bg-gradient-to-br from-orange-700 to-orange-800 w-28 h-28 rounded-2xl flex items-center justify-center mb-4 border border-orange-700/30 shadow-2xl">
                            ...
                            <img :src="props.classement[2].avatar || 'https://via.placeholder.com/100x100.png/CD7F32?text=3'" 
                                 class="w-16 h-16 rounded-full border-3 border-orange-700 mb-2 transform transition-all duration-300 hover:scale-110 hover:rotate-6" />
                            <p class="text-white font-light text-lg">{{ props.classement[2].name }}</p>
                            <p class="text-slate-400 text-sm">{{ props.classement[2].pseudo }}</p>
                            <div class="mt-2">
                                <p class="text-slate-300 text-sm">B: {{ props.classement[2].total_goals }} | P: {{ props.classement[2].total_assists }}</p>
                                <p class="text-2xl font-thin text-white mt-1">{{ props.classement[2].points }} pts</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="props.classement.length > 0" class="bg-slate-900/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-slate-800 shadow-2xl animate-slide-up">
                        ...
                        <div v-for="(joueur, index) in props.classement" 
                            ...
                            <div class="grid grid-cols-12 gap-6 items-center">
                                ...
                                <div class="col-span-5 flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="font-light text-white uppercase tracking-wide text-lg">{{ joueur.name }}</p>
                                        <p class="text-slate-400 text-sm">{{ joueur.pseudo }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="col-span-2 text-center">
                                    <div class="bg-slate-800/50 rounded-xl px-4 py-3 border border-slate-700 transform transition-all duration-300 hover:bg-slate-700/50 hover:scale-105">
                                        <span class="text-white font-light text-2xl">{{ joueur.points }}</span>
                                    </div>
                                </div>
                ...
                <div v-if="props.classement.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
                    <div class="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-8 text-center border border-slate-800 transform transition-all duration-300 hover:scale-105 hover:bg-slate-800/50 animate-slide-up-delay">
                        ...
                        <p class="text-4xl font-thin text-yellow-500">{{ props.classement[0]?.points || 0 }}</p>
                        <p class="text-slate-400 text-sm tracking-widest font-light mt-2">LEADER POINTS</p>
                        <p class="text-white mt-2 font-light">{{ props.classement[0]?.name || '-' }}</p>
                    </div>
                    ...
                    <div class="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-8 text-center border border-slate-800 transform transition-all duration-300 hover:scale-105 hover:bg-slate-800/50 animate-slide-up-delay-2">
                        ...
                        <p class="text-slate-400 text-sm tracking-widest font-light mt-2">TOTAL POINTS</p>
                    </div>
                    ...
                    <div class="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-8 text-center border border-slate-800 transform transition-all duration-300 hover:scale-105 hover:bg-slate-800/50 animate-slide-up-delay-3">
                        ...
                        <p class="text-4xl font-thin text-slate-300">{{ props.classement.length }}</p>
