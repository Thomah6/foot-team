...
const ANIMATION_DURATION = 2500 // ms, decrement duration (4s)
...
function animateMontantDecrement() {
  // If user already interacted, skip animation
  ...
  function step(now) {
    ...
    if (progress < 1 && isAnimating.value) {
      ...
      // ensure exact min at the end
      montant.value = end
      isAnimating.value = false
    }
  }

  // Start from max so slider visual updates
  montant.value = MAX_MONTANT
  requestAnimationFrame(step)
}

// Cancel animation if user moves the slider manually
function onUserInteract() {
  isAnimating.value = false
}

onMounted(() => {
  // small timeout so the component mount transition is visible
  setTimeout(() => {
...
<template>
  <div class="deposit-animate flex flex-col gap-4 p-6 bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-700 rounded-lg">
    ...
    <div class="flex flex-col items-center gap-4">
      ...
      <input type="range" min="100" max="10000" step="100" v-model.number="montant" @input="onUserInteract" class="w-full h-2 bg-neutral-200 dark:bg-slate-700 rounded-lg accent-primary dark:accent-blue-500"/>
...
/* subtle focus/glow for the montant when animating */
.deposit-animate .text-5xl {
  transition: transform 220ms ease, color 220ms ease;
}
/* small press effect on send button */
